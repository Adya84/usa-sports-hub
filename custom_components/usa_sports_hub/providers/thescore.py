"""Unified theScore provider for NFL, NBA, MLB and NHL."""
...
from .models import (
    compact_the_score_news,
    normalize_the_score_event,
    normalize_the_score_standing,
    normalize_the_score_team,
)


API_BASE = "https://api.thescore.com"
WEB_API_BASE = "https://www.thescore.com/api"
...
class TheScoreProvider(ProviderClient):
    ...
    async def async_schedule(self) -> list[dict[str, Any]]:
        ...
        if not isinstance(payload, list):
            return []
        games = [
            normalize_the_score_event(item, self.league)
            for item in payload
    ...
    async def async_news(self) -> list[dict[str, Any]]:
        ...
        return compact_the_score_news(payload if isinstance(payload, list) else [])
        ...
        if isinstance(standings, list):
            for row in standings:
                ...
                if isinstance(team, dict) and team.get("id") is not None:
                    normalized = normalize_the_score_team(team)
    ...
    async def async_game_detail(self, game_id: str | int) -> dict[str, Any]:
        ...
        detail: dict[str, Any] = {
            "event": normalize_the_score_event(event, self.league),
...
class NflProvider(TheScoreProvider):
    def __init__(self, session):
        super().__init__(session, "nfl")


class NbaProvider(TheScoreProvider):
    def __init__(self, session):
        super().__init__(session, "nba")


class MlbProvider(TheScoreProvider):
    def __init__(self, session):
        super().__init__(session, "mlb")


class NhlProvider(TheScoreProvider):
    def __init__(self, session):
