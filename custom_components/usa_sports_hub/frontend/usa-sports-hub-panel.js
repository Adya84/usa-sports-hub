...
class UsaSportsHubPanel extends HTMLElement {
  ...
  mlbGameCentre(s,items,sensor){
    ...
    const batter=this.pick(situation,['batter'])||'';
    const pitcher=this.pick(situation,['pitcher'])||'';
    const bases=[
    ...
    const playerSource=(lineups.length?lineups:players).filter(Boolean);
    const playerCards=playerSource.slice(0,60).map(x=>{
      const name=this.pick(x,['full_name','name','first_initial_and_last_name','player_name'])||'Player';
      const pos=this.pick(x,['position_abbreviation','position','starting_position'])||'';
      const num=this.pick(x,['jersey_number','number'])||'';
      const team=this.pick(x,['team_name','team'])||'';
      return '<div class="mlb-player"><div><b>'+esc(name)+'</b><small>'+esc([pos,num&&('#'+num),team].filter(Boolean).join(' · '))+'</small></div></div>';
    }).join('');

    const statRows=stats.filter(x=>x&&typeof x==='object').slice(0,24).map(x=>{
      const name=this.pick(x,['full_name','name','team_name','player_name','label','stat_name'])||'';
      const useful=this.scalarPairs(x).filter(([k])=>!['rank','formatted_rank','league_rank','conference_rank','division_rank','batting_average_against_rank'].includes(k)).slice(0,5);
      if(!name&&!useful.length)return '';
      return '<div class="mlb-stat-row"><b>'+esc(name||this.prettyKey(useful[0]?.[0]||'Stat'))+'</b><div>'+useful.map(([k,v])=>'<span><small>'+esc(this.prettyKey(k))+'</small><strong>'+esc(v)+'</strong></span>').join('')+'</div></div>';
    }).filter(Boolean).join('');
