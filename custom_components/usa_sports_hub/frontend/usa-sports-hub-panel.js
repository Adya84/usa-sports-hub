...
class UsaSportsHubPanel extends HTMLElement {
  ...
  mlbGameCentre(s,items,sensor){
    ...
    const battingRows=battingStats.filter(x=>x.full_name).slice(0,12).map(x=>
      '<div class="mlb-leader-row"><div class="mlb-avatar">'+esc((x.full_name||'?').charAt(0))+'</div><div class="mlb-leader-name"><b>'+esc(x.full_name)+'</b><small>'+esc([x.team_name,x.position_abbreviation].filter(Boolean).join(' · '))+'</small></div>'+
    ...
    const scoreSummary=scoring.slice(0,40).map(x=>{
      ...
      return '<div class="mlb-score-event"><div class="mlb-score-inning">'+esc(inning)+'</div><div class="mlb-score-event-body"><b>'+esc(text)+'</b></div></div>';
    }).join('');
