...
class UsaSportsHubPanel extends HTMLElement {
  ...
  mlbPlayerPhoto(player,sizeClass){
    ...
    return url
      ? '<img class="mlb-player-photo '+esc(sizeClass||'')+'" src="'+esc(url)+'" alt="'+esc(name)+'" loading="lazy" referrerpolicy="no-referrer">'
      : '<div class="mlb-avatar '+esc(sizeClass||'')+'">'+esc(String(name||'?').charAt(0))+'</div>';
  }
    ...
    return '<section class="mlb-centre reference-style">'+
      ...
      '<div class="mlb-reference-grid">'+
        ...
        '<aside class="mlb-side-column">'+
          ...
          '</section>'+
          '<section class="mlb-side-card"><h3>Current At Bat</h3><div><b>'+esc(batter||'—')+'</b><span>'+esc(pitcher?('vs '+pitcher):'')+'</span></div><div><b>'+esc(bases)+'</b><span>'+esc((situation.balls??0)+'-'+(situation.strikes??0)+' · '+(situation.outs??0)+' outs')+'</span></div></section>'+
