...
class UsaSportsHubPanel extends HTMLElement {
  ...
  mlbGameCentre(s,items,sensor){
    ...
    if(validInnings.length){
      const heads=validInnings.map(x=>'<th>'+esc(x.inning)+'</th>').join('');
      ...
      lineScore='<section class="mlb-panel wide"><header><div><small>GAME</small><h3>Line Score</h3></div></header><div class="mlb-table-wrap"><table class="mlb-linescore"><thead><tr><th>TEAM</th>'+heads+'<th>R</th><th>H</th><th>E</th></tr></thead><tbody><tr><th>'+esc(game.away_abbreviation||game.away_team||'Away')+'</th>'+awayCells+'<td class="total">'+esc(awayTotals.r)+'</td><td>'+esc(awayTotals.h)+'</td><td>'+esc(awayTotals.e)+'</td></tr><tr><th>'+esc(game.home_abbreviation||game.home_team||'Home')+'</th>'+homeCells+'<td class="total">'+esc(homeTotals.r)+'</td><td>'+esc(homeTotals.h)+'</td><td>'+esc(homeTotals.e)+'</td></tr></tbody></table></div></section>';
    }

    const playRows=plays.slice(-80).reverse().map(p=>{
      const inning=[p.half_inning,p.inning&&(''+p.inning)].filter(Boolean).join(' ');
      const matchup=[p.batter&&('Batter: '+p.batter),p.pitcher&&('Pitcher: '+p.pitcher)].filter(Boolean).join(' · ');
      const count=(p.balls!==undefined||p.strikes!==undefined||p.outs!==undefined)?((p.balls??0)+'-'+(p.strikes??0)+' · '+(p.outs??0)+' out'+((p.outs??0)===1?'':'s')):'';
      return '<div class="mlb-play"><div class="mlb-play-meta"><b>'+esc(inning||'Play')+'</b><span>'+esc(count)+'</span></div><p>'+esc(p.description||p.event||'Play update')+'</p>'+(matchup?'<small>'+esc(matchup)+'</small>':'')+'</div>';
    }).join('');

    const looksLikePlayer=x=>{
      if(!x||typeof x!=='object')return false;
      const name=this.pick(x,['full_name','first_initial_and_last_name','player_name','name']);
      const pos=this.pick(x,['position_abbreviation','position_name','position','starting_position']);
      const jersey=this.pick(x,['jersey_number','number']);
      const batting=this.pick(x,['batting_order','batting_spot']);
      const teamName=String(this.pick(x,['team_name','team'])||'').toLowerCase();
      const n=String(name||'').toLowerCase();
      if(!name)return false;
      if(n==='mlb baseball'||n==='major league baseball'||n===String(game.home_team||'').toLowerCase()||n===String(game.away_team||'').toLowerCase())return false;
      if(/umpire|official|referee/.test(String(pos||'').toLowerCase()))return false;
      return Boolean(pos||jersey||batting||teamName);
    };
    const playerSource=(lineups.length?lineups:players).filter(looksLikePlayer);
    const playerCards=playerSource.slice(0,60).map(x=>{
      const name=this.pick(x,['full_name','first_initial_and_last_name','player_name','name'])||'Player';
      const pos=this.pick(x,['position_abbreviation','position_name','position','starting_position'])||'';
      const num=this.pick(x,['jersey_number','number'])||'';
      const team=this.pick(x,['team_name','team'])||'';
      const order=this.pick(x,['batting_spot']);
      return '<div class="mlb-player">'+(order?'<span class="bat-order">'+esc(order)+'</span>':'')+'<div><b>'+esc(name)+'</b><small>'+esc([pos,num&&('#'+num),team].filter(Boolean).join(' · '))+'</small></div></div>';
    }).join('');

    const statRows=stats.filter(x=>x&&typeof x==='object').slice(0,40).map(x=>{
      const group=String(this.pick(x,['group'])||'').toLowerCase();
      const playerName=this.pick(x,['full_name','player_name']);
      const teamName=this.pick(x,['team_name']);
      const name=playerName||teamName||this.pick(x,['name','label','stat_name'])||'';
      const ignored=new Set(['id','rank','formatted_rank','league_rank','conference_rank','division_rank','batting_average_against_rank','alignment','batting_slot','game_status','side','group','team_name','full_name','name','position_abbreviation']);
      const useful=this.scalarPairs(x).filter(([k,v])=>!ignored.has(k)&&typeof v!=='object').slice(0,8);
      if(!name||!useful.length)return '';
      const heading=[name,group?group.charAt(0).toUpperCase()+group.slice(1):''].filter(Boolean).join(' · ');
      return '<div class="mlb-stat-row"><b>'+esc(heading)+'</b><div>'+useful.map(([k,v])=>'<span><small>'+esc(this.prettyKey(k))+'</small><strong>'+esc(v)+'</strong></span>').join('')+'</div></div>';
    }).filter(Boolean).join('');

    const scoringRows=scoring.slice(0,30).map(x=>{
      const title=this.pick(x,['description','text','play_description','event','name','label'])||this.itemTitle(x);
      const meta=[this.pick(x,['inning','segment_string','clock']),this.pick(x,['team_name'])].filter(Boolean).join(' · ');
      return '<div class="mlb-summary-row"><b>'+esc(title)+'</b>'+(meta?'<small>'+esc(meta)+'</small>':'')+'</div>';
    }).join('');

    const venue=[game.venue,game.location,stadium.name,stadium.city,stadium.state].filter(Boolean);
    return '<section class="mlb-centre">'+
      ...
      '<section class="mlb-score-hero"><div class="mlb-team">'+this.gameLogo(game.away_logo,game.away_team)+'<h2>'+esc(game.away_team||'Away')+'</h2><small>'+esc(game.away_abbreviation||'')+'</small></div><div class="mlb-score-core"><span>'+esc(inningLabel)+'</span><strong>'+esc(game.away_score??'–')+' <i>–</i> '+esc(game.home_score??'–')+'</strong><small>'+esc(game.status_detail||'')+'</small></div><div class="mlb-team">'+this.gameLogo(game.home_logo,game.home_team)+'<h2>'+esc(game.home_team||'Home')+'</h2><small>'+esc(game.home_abbreviation||'')+'</small></div></section>'+
      (venue.length?'<div class="venue-strip">'+esc([...new Set(venue)].join(' · '))+'</div>':'')+
      '<section class="mlb-now"><div><small>CURRENT</small><b>'+esc(inningLabel||'Game in progress')+'</b></div><div><small>COUNT</small><b>'+esc((situation.balls??game.balls??0)+'-'+(situation.strikes??game.strikes??0))+'</b><span>'+esc((situation.outs??game.outs??0)+' out'+((situation.outs??game.outs??0)===1?'':'s'))+'</span></div><div><small>BASES</small><b>'+esc(bases)+'</b></div><div><small>AT BAT</small><b>'+esc(batter||'—')+'</b><span>'+esc(pitcher?('vs '+pitcher):'')+'</span></div></section>'+
      lineScore+
      '<div class="mlb-content-grid">'+
        (playRows?'<section class="mlb-panel"><header><div><small>LIVE</small><h3>Play-by-Play</h3></div></header><div class="mlb-scroll">'+playRows+'</div></section>':'')+
        (statRows?'<section class="mlb-panel"><header><div><small>GAME</small><h3>Player & Team Stats</h3></div></header><div class="mlb-scroll">'+statRows+'</div></section>':'')+
        (scoringRows?'<section class="mlb-panel"><header><div><small>GAME</small><h3>Scoring Summary</h3></div></header><div class="mlb-scroll">'+scoringRows+'</div></section>':'')+
        (playerCards?'<section class="mlb-panel"><header><div><small>ROSTER</small><h3>Lineups & Players</h3></div></header><div class="mlb-player-grid">'+playerCards+'</div></section>':'')+
      '</div>'+
      ((!playRows&&!lineScore&&!statRows&&!playerCards)?'<div class="empty-live large">Detailed MLB game data is still loading for this game.</div>':'')+
    '</section>';
  }
